...
export default function TasksCenter() {
  ...
  const saveTask = () => {
    ...
    if (editingId) {
      ...
      notify('Task Updated', `"${draft.title}" was updated`, 'orange', '/tasks');
      setEditingId(null);
    } else {
      const task: Task = {
        id: `TSK-${Date.now()}`,
  ...
  const updateTask = (id: string, updates: Partial<Task>) => {
    ...
    if (updates.completed === true) {
      ...
      if (task) notify('Task Completed', `"${task.title}" marked complete`, 'green', '/tasks');
    }
  };

  const deleteTask = (id: string) => {
    const task = tasks.find(t => t.id === id);
    setTasks(dataService.deleteTask(id));
              ...
              {filteredTasks.map((task) => {
                ...
                return (
                  <tr key={task.id} className={cn('group hover:bg-accent/40', overdue && 'bg-red-50/60 dark:bg-red-900/10')}>
                    ...
                    <td className="px-5 py-4">
                      <select className="settings-input min-w-36" value={ownerId} onChange={(event) => updateTask(task.id, { ownerId: event.target.value })}>
                        {[ownerId, ...owners].filter((owner, index, list) => owner && list.indexOf(owner) === index).map((owner) => <option key={owner} value={owner}>{owner}</option>)}
                      </select>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      <div className={cn('flex items-center gap-2 text-xs font-bold', overdue ? 'text-red-600' : 'text-muted-foreground')}>
                        <Clock size={14} />
                        {formatDueDate(task.dueDate, 'MMM dd, yyyy')}
                      </div>
                    </td>
                    <td className="px-5 py-4 text-right">
                      {confirmDeleteId === task.id ? (
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button onClick={() => deleteTask(task.id)} className="px-2 py-1 bg-destructive text-white rounded text-[10px] font-bold hover:bg-destructive/90">Yes</button>
                        ...
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100">
                          <button onClick={() => startEdit(task)} className="icon-btn"><Edit2 size={15} /></button>
                          <button onClick={() => setConfirmDeleteId(task.id)} className="icon-btn text-destructive hover:bg-destructive/10"><Trash2 size={15} /></button>
                        </div>
                      )}
                    </td>
                  </tr>
