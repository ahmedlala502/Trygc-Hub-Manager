...
const roleToLocalAuthRole = (role: AdminRole): LocalAuthRole => (role === 'Owner' || role === 'Super Admin' ? 'admin' : 'user');

const mergeLocalAuthUsers = (directoryUsers: AdminUser[]) => {
  const usersByEmail = new Map(directoryUsers.map(user => [user.email.toLowerCase(), user]));
  ...
  localAuth.listUsers().forEach(localUser => {
    ...
    usersByEmail.set(email, {
      ...
      role: localUser.role === 'admin' ? 'Super Admin' : 'Viewer',
      access: localUser.role === 'admin' ? 'Full' : 'Scoped',
      status: 'Active',
...
export default function Admin() {
  ...
  const [newUser, setNewUser] = useState({ name: '', email: '', password: '', role: 'Viewer' as AdminRole, access: 'Scoped' as AccessLevel });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed.users)) setUsers(mergeLocalAuthUsers(parsed.users));
  ...
  const createUser = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setNewUser({ name: '', email: '', password: '', role: 'Viewer', access: 'Scoped' });
