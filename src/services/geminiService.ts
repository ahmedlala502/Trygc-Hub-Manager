...
export function getDiscoveryProviderConfig(): AiProviderConfig {
  ...
  const base: AiProviderConfig = {
    ...
    apiKey: envValue(fallbackPreset.envKey),
    mode: envValue(fallbackPreset.envKey) ? 'live' : 'demo',
    temperature: 0.35,
  };
  ...
  try {
    ...
    const apiKey = (ai.apiKey || envValue(preset.envKey) || '').trim();
    return {
      provider: ai.provider,
      model: ai.model || preset.model,
      endpoint: ai.endpoint || preset.endpoint,
      apiKey,
      mode: ai.mode || (apiKey ? 'live' : 'demo'),
